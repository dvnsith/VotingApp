...
if(!hasVoted) {
    update.addEventListener('click', _ => {
        ...
        .then(() => button[0].innerHTML = `Votes: ${button[0].getAttribute('value')} / ${parseInt(button2[0].getAttribute('value')) + parseInt(button[0].getAttribute('value'))}`)
    ...
    update2.addEventListener('click', _ => {
        ...
        .then(data => console.log(data))
        .then(() => localStorage.setItem("food photo", "voted"))
        .then(() => button2[0].innerHTML = `Votes: ${button2[0].getAttribute('value')} / ${parseInt(button2[0].getAttribute('value')) + parseInt(button[0].getAttribute('value'))}`)
